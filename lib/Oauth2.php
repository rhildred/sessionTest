...
class Oauth2
{
	...
	public function handleCode($code)
	{
		...
		$oCurrent = new CurrentUser();
		if(!$oCurrent->load("id = ? OR email = ?", array($oProfile -> id, $oProfile -> email)) != 0
				&& $oProfile->verified_email == 1 )
		{
			$oProfile->bNew = true;
		}
		elseif ($oProfile->verified_email != 1)
		{
			throw new ControllerException(401, "Unauthorized");
		}
		else
		{
			$oProfile->id = $oCurrent->id;
		}
		return $oProfile;
